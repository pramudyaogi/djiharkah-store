...
export default function ProductDetail() {
  ...
  useEffect(() => {
    async function fetchProductAndReviews() {
      try {
        ...
        // Fetch Store Settings for Origin City & WhatsApp Phone
        const { data: settingsData, error: settingsError } = await supabase
          .from('store_settings')
          .select('shipping_origin_city, phone')
          .eq('id', 1)
          .single();
  ...
  const handleWhatsAppInquiry = () => {
    ...
    const priceStr = formatPrice(product.price, currency, rates);
    const message = t('wa_inquiry_greeting', { name: product.name, price: priceStr });
  ...
  if (!product) return <div className="text-hitam font-playfair text-xl py-20 text-center">{t('product_not_found')}</div>;
    ...
    <div className="max-w-[1200px] mx-auto pt-6 px-4 pb-20">
      ...
      <div className="bg-white p-6 md:p-10 rounded-3xl shadow-soft flex flex-col md:flex-row gap-10 mb-8 border border-gray-100">
          ...
          <div className="w-full pt-[100%] relative bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 shadow-sm group">
            ...
            {product.promo_type === 'flash_sale' && (
              <div className="absolute top-4 left-4 bg-red-600 text-white text-[10px] md:text-xs font-black uppercase tracking-wider px-3 py-1.5 rounded-md shadow-md z-10 flex items-center gap-1.5 border border-red-500/30 animate-pulse">
                ⚡ {t('flash_sale')} {product.discount_percent > 0 && `-${product.discount_percent}%`}
              </div>
            )}
            {product.promo_type === 'custom_promo' && (
              <div className="absolute top-4 left-4 bg-amber-500 text-white text-[10px] md:text-xs font-black uppercase tracking-wider px-3 py-1.5 rounded-md shadow-md z-10 flex items-center gap-1.5 border border-amber-400/30 animate-pulse">
                ✨ {t('special_promo')} {product.discount_percent > 0 && `-${product.discount_percent}%`}
              </div>
            )}
            {!product.promo_type && product.discount_percent > 0 && (
              <div className="absolute top-4 left-4 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md z-10 animate-pulse">
                {t('discount').toUpperCase()} {product.discount_percent}%
              </div>
            )}
          </div>
        ...
        <div className="flex-1 py-2 flex flex-col">
          ...
          <div className="flex items-center gap-4 text-sm mb-4">
            <div className="text-hitam"><span className="font-bold">{soldCount}</span> {t('sold')}</div>
          </div>
          ...
          <div className="flex flex-col gap-6 mb-8 flex-1">
            <div className="flex items-start gap-6">
              <span className="text-gray-500 w-24 shrink-0 text-sm mt-0.5">{t('shipping')}</span>
              <div className="flex flex-col gap-1.5 text-sm text-hitam">
                <div className="flex items-center gap-2 font-medium">
                  <span>{t('shipped_from')}</span>
            ...
            <div className="flex items-center gap-6">
              <span className="text-gray-500 w-24 shrink-0 text-sm">{t('quantity')}</span>
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-gray-500">
                  {t('remaining')} {product.promo_type ? product.original_stock : product.stock} {t('units')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              className="w-full bg-white hover:bg-zinc-50 text-hitam border-2 border-hitam py-3.5 rounded-full font-bold text-lg hover:-translate-y-1 hover:shadow-[0_10px_20px_rgba(24,24,27,0.1)] transition-all duration-300 flex items-center justify-center gap-2"
            >
      ...
      <div className="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden mb-10">
        <div className="p-6 md:p-8 bg-gray-50/50 border-b border-gray-100">
          <h2 className="text-lg font-bold text-hitam-gelap uppercase tracking-wide">{t('specifications_desc')}</h2>
        </div>
        <div className="p-6 md:p-8 text-sm text-gray-700">
          <div className="grid grid-cols-2 max-w-sm gap-y-4 mb-8">
            ...
            <div className="text-gray-500">{t('stock')}</div>
            <div>
              <span>{product.promo_type ? product.original_stock : product.stock} {t('units')}</span>
            </div>
            <div className="text-gray-500">{t('shipped_from_label')}</div>
            <div>{originCity || 'Jakarta Barat'}</div>
          </div>
          <div className="whitespace-pre-line leading-relaxed text-gray-700">
            {(language === 'EN' ? (product.description_en || product.description) : product.description) || t('no_description')}
          </div>
        </div>
      </div>
